...
export const setIn = (path: string, data: any) => firebase.database().ref(path).set(data)

export const push = (path: string, data: any) => firebase.database().ref(path).push().set(data)

export const remove = (path: string) => firebase.database().ref(path).remove()

const getOffFunc = (path: string, eventType: EventType, callback: FBCallback) => {
    return () => {
